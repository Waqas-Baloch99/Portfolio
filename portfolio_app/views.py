...
def portfolio_view(request):
    context = {
        'profile_image': request.user.profile.image if hasattr(request.user, 'profile') else None,
        'user': request.user,
        'experience': request.user.profile.experience if hasattr(request.user, 'profile') else 0,
        'signup_year': request.user.date_joined.year,
        'linkedin_url': request.user.profile.linkedin_url if hasattr(request.user, 'profile') else None,
        'github_url': request.user.profile.github_url if hasattr(request.user, 'profile') else None,
